...
<div class="field has-addons">
    <div class="control is-expanded">
        <input type="search" id="search-input" placeholder="Suchen Sie hier" class="input is-large">
    </div>
</div>
...
        const indexes = {};
        const loadedLanguagePacks = new Set();
...
            // Only use the language pack if it was successfully loaded
            const encoderConfig = loadedLanguagePacks.has(lang) ? languagePack : FlexSearch.Charset.LatinSoundex;
...
            // Load language pack script
            promises.push(new Promise(resolve => {
                const script = document.createElement('script');
                script.src = languagePackUrl;
                script.onload = () => {
                    loadedLanguagePacks.add(lang);
                    resolve();
                };
                script.onerror = () => {
                    console.warn(`FlexSearch language pack not available for "${lang}" at ${languagePackUrl}`);
                    resolve(); // Resolve anyway to not block the main process
                };
                document.head.appendChild(script);
            }));

            // Load JSON data
            promises.push(fetch(jsonUrl).then(response => {
                if (!response.ok) {
...
                const index = createIndex(lang, FlexSearch.lang[lang]);
                data.forEach(item => {
                    if (item.url) {
                        index.add(item);
                    } else {
