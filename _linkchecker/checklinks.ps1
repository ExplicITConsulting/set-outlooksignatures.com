    ...
    1..$ParallelWorkers | ForEach-Object -Parallel {
        try {
            ...
            $launchOptions = [Microsoft.Playwright.BrowserTypeLaunchOptions]@{
                Headless = $false # We use --headless=new in Args for better stealth
                Args     = [string[]]@(
                    '--headless=new',
...
'@
                        ...
                        $nodes = $htmldoc.DocumentNode.SelectNodes('//*[@href or @src or @data-href or @srcset or @data-src or @data-srcset or @action or @poster or @cite]')
                        ...
                        if ($null -ne $nodes) {
                            ...
                            foreach ($node in $nodes) {
                                ...
                                foreach ($setAttr in $setAttrs) {
                                    $srcsetValue = $node.GetAttributeValue($setAttr, '')
                                    if ($srcsetValue) {
