...
            
            // **CRITICAL: PHRASE SEARCH LOGIC**
            let strictQuery = rawQuery; // Default to raw query
            
            if (isPhraseSearch) {
                const unquotedQuery = rawQuery.slice(1, -1);
                const words = unquotedQuery.split(/\s+/).filter(w => w.length > 0);

                if (words.length > 1) {
                    // Transform into a proximity search query: "word1~0 word2~0 word3"
                    // The '~0' operator enforces that words must be adjacent and in order.
                    strictQuery = words.join("~0 "); 
                } else {
                    // If only one word is quoted, search for just that word
                    strictQuery = unquotedQuery;
                }
            }
            // **END CRITICAL CHANGE**
...
            const processResults = (index, lang, scoreAdjustment, queryToUse) => {
                // NOTE: queryToUse is rawQuery for 'full' search, and strictQuery for 'strict' search.
                const results = index.search(queryToUse, searchOptions);
                
...
            
            // Use the transformed strictQuery (which includes ~0 for phrases)
            processResults(indexes[currentLang].strict, currentLang, PRIORITY_ADJUSTMENT, strictQuery);

            // --- 2. SEARCH THE FULL INDEX (General Flexibility) ---
            // Use the raw (unmodified) query for general search
            processResults(indexes[currentLang].full, currentLang, 0, rawQuery);
            
...
                    processResults(indexes[lang].full, lang, 0, rawQuery);
                }
            });
