...
When the '`manifest.xml`' file, the configuration or another part of the Outlook add-in changes, you need to tell your mailboxes that an updated version or configuration is available and must be downloaded from the web server. The "[Deployment to mailboxes](#5-deployment-to-mailboxes)" chapter describes the available deployment options for add-ins.
