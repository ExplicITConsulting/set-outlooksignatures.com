...
* **Targeting:** Applied to all **mailboxes (including [shared mailboxes](/benefactorcircle#key-features)¹)**, specific **[mailbox groups](/details#template-tags-and-ini-files)**, specific **[email addresses](/details#template-tags-and-ini-files)** (including alias and secondary addresses), or specific **[user and mailbox properties](/details#template-tags-and-ini-files)**.
* **Outlook Support:** Works for **every mailbox across all Outlook profiles (Outlook, [New Outlook](/benefactorcircle#key-features)¹, [Outlook on the web](/benefactorcircle#key-features)¹)**, including **[automapped and additional mailboxes](/parameters#signaturesforautomappedandadditionalmailboxes)**¹.
...
* **Scheduling:** Assigned **[time ranges](/details#allowed-tags)**¹ within which they are valid.
* **Defaults:** Set as **[default signature](/details#allowed-tags)** for new emails, replies, and forwards, or as **[default OOF message](/details#allowed-tags)**¹ for internal/external recipients.

### 💻 Platform & Execution
* **Cloud Mirroring:** Set in **[Outlook on the web](/parameters#setcurrentuseroutlookwebsignature)**¹ for the currently logged-in user, including mirroring signatures to the cloud as **[roaming signatures](/parameters#mirrorcloudsignatures)**¹ (Linux/macOS/Windows, Classic and New Outlook¹).
* **User Control:** Signatures can be centrally managed only¹, or **[exist along user-created signatures](/parameters#deleteusercreatedsignatures)**.
* **Add-in Integration:** Automatically added to new emails, reply emails and appointments with the **[Outlook add-in](/outlookaddin)**¹.
* **Mobile & Third-Party:** Copied to an **[additional path](/parameters#additionalsignaturepath)**¹ for easy access on mobile devices or use with Apple Mail, Google Gmail, Samsung Mail, Mozilla Thunderbird, GNOME Evolution, KDE KMail, and others.
* **Drafts API Fallback:** Create an **[email draft containing all available signatures](/parameters#signaturecollectionindrafts)** in HTML and plain text for mail clients without a signatures API.
* **Security:** **[Write protected](/details#allowed-tags)** (Outlook on Windows signatures only).

### 🛡️ Architecture & Infrastructure
* **Execution Flexibility:** **[Run by users on Windows, Linux and macOS clients (including shared devices/terminal servers) — or on a central system with a service account](/details#architecture-considerations)**¹.
...
* **Zero-Touch:** **[SimulateAndDeploy](/details#creating-signatures-and-out-of-office-replies)**¹ allows deployment **[without any client deployment or end user interaction](/details#creating-signatures-and-out-of-office-replies)** (roaming signatures for Exchange Online).
...
* **Open Source:** **[Free and Open-Source (FOSS)](/faq#why-the-tagline)** under the European Union Public License (EUPL). After a certain period, a tagline is appended which can be easily removed¹.
